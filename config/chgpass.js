...
var smtpTransport = nodemailer.createTransport("SMTP",{     
     auth: {         
            user: "greidanapp@gmail.com",        
            pass: "slakurideildinni"        
          } 
});   
    ...
    user.find({token: id},function(err,users){  
            ...
            if(hash_db == hashed_password){ 
                if (npass.match(/([a-z].*[A-Z])|([A-Z].*[a-z])/) && npass.length > 4 && 
                    ...
                    callback({'response':"New Password is Weak. Try a Strong Password !",
                        'res':false});  
    ...
    user.find({email: email},function(err,users){  
        ...
        if(users.length != 0){   
            ...
            user.findOne({ email: email }, function (err, doc){   
                ...
                var mailOptions = {     
                     from: "Greiðan app <greidanapp@gmail.com>",     
                     to: email,     
                ...
                smtpTransport.sendMail(mailOptions, function(error, response){     
                     if(error){  
                        ...
                        callback({'response':"Error While Resetting password. Try Again !",
                            'res':false});      
